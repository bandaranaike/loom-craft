...
const formatDimensions = (dimensions: ProductDetails['dimensions']) => {
    const parts = [dimensions.length, dimensions.width, dimensions.height].filter((value) => value !== null) as number[];
...
const renderRatingStars = (rating: number, filledClassName = 'text-amber-500', emptyClassName = 'text-(--welcome-border)') => {
    return Array.from({ length: 5 }, (_, index) => {
        const starNumber = index + 1;
        const isFilled = starNumber <= rating;

        return (
            <span key={starNumber} aria-hidden="true" className={isFilled ? filledClassName : emptyClassName}>
                ★
            </span>
        );
    });
...
export default function ProductShow({
    ...
    useEffect(() => {
        ...
        const updateThumbnailDock = (): void => {
            ...
            const tolerance = 8;
            const isGalleryVisible = frameRect.top < viewportBottom && frameRect.bottom > 0;
            ...
            if (isScrollingDown && previousBottom !== null && previousBottom > viewportBottom + tolerance && frameRect.bottom <= viewportBottom + tolerance) {
                setIsThumbnailDocked(true);

                return;
            }

            if (!isScrollingDown && frameRect.bottom > viewportBottom + tolerance) {
                setIsThumbnailDocked(false);
            }
        };
                ...
                {product.images.map((image, index) => {
                    ...
                    return (
                        <button
                            ...
                            className={`h-16 w-16 cursor-pointer overflow-hidden rounded-lg transition-opacity ${isSelected ? 'opacity-100' : 'opacity-60 hover:opacity-100'}`}
                ...
                <section className="relative z-10 mx-auto grid w-full max-w-6xl gap-10 px-6 pt-6 pb-16 lg:grid-cols-[1.05fr_0.95fr]">
                    <div className="grid gap-6">
                        <div className="grid gap-4">
                            ...
                            >
                                {selectedImage ? (
                                    <>
                                        <img src={selectedImage.url} alt={selectedImage.alt_text ?? product.name} className="h-auto w-full rounded-4xl object-contain" />
                                        {hasMultipleImages && (
                                            <>
                                                <button
                                                    type="button"
                                                ...
                                                {!isThumbnailDocked && <div className="fixed bottom-0 z-20 hidden justify-start p-4 md:flex">{renderThumbnails()}</div>}
                                            </>
                                        )}
                                    </>
                                ) : (
                            ...
                            {hasMultipleImages && (
                                <div className="relative min-h-16">
                                    <div className={!isThumbnailDocked ? 'md:pointer-events-none md:opacity-0' : 'transition-opacity duration-200'}>{renderThumbnails()}</div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <h1 className="font-['Playfair_Display',serif] text-4xl leading-tight md:text-5xl">{product.name}</h1>
                            <p className="mt-3 text-sm tracking-[0.35em] text-(--welcome-muted-text) uppercase">
                                ...
                                {product.vendor.location ? ` • ${product.vendor.location}` : ''}
                            </p>
                            <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-(--welcome-body-text)">
                                <div className="flex items-center gap-1 text-base">{renderRatingStars(Math.round(Number(review_summary.average_rating ?? 0)))}</div>
                        ...
                        <div className="rounded-4xl border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-6">
                            <div className="flex items-center justify-between gap-3">
                                <p className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase">Selling Price</p>
                                        ...
                                        {product.variations.map((variation) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={
                                                        isSelected
                                                            ? 'rounded-full border border-(--welcome-strong) bg-(--welcome-strong) px-4 py-2 text-xs font-semibold tracking-[0.2em] text-(--welcome-on-strong) uppercase'
                                                            : 'rounded-full border border-(--welcome-border) bg-(--welcome-surface-3) px-4 py-2 text-xs font-semibold tracking-[0.2em] text-(--welcome-strong) uppercase transition hover:border-(--welcome-strong)'
                                                    }
                                                >
                                                    {variation.label}
                                                </button>
                        ...
                        <div className="grid gap-4 rounded-4xl border border-(--welcome-border-soft) bg-(--welcome-surface-1) p-6">
                            ...
                            <div className="flex items-center justify-between gap-4 text-sm">
                                <span className="tracking-[0.25em] text-(--welcome-muted-text) uppercase">Dimensions</span>
                                <span className="text-(--welcome-strong)">{dimensionLabel ?? 'Dimensions on request'}</span>
                            </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-4">
                            <button
                    ...
                    <div className="space-y-6">
                        <div className="rounded-[40px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-8">
                            ...
                            <div className="mt-4 flex items-end gap-4">
                                ...
                                <div className="pb-2">
                                    <div className="flex items-center gap-1 text-lg">{renderRatingStars(Math.round(Number(review_summary.average_rating ?? 0)))}</div>
                                    <p className="mt-2 text-sm text-(--welcome-body-text)">
                        ...
                        <div className="rounded-[40px] border border-(--welcome-border-soft) bg-(--welcome-surface-1) p-8">
                            <p className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase">Review eligibility</p>
                            <h2 className="mt-3 font-['Playfair_Display',serif] text-2xl">Verified after fulfillment</h2>
                            <p className="mt-3 text-sm leading-6 text-(--welcome-body-text)">
                                Ratings are reserved for customers whose order has been fulfilled, keeping product feedback grounded in completed purchases.
                            </p>
                            {review_form.message && <p className="mt-4 rounded-3xl bg-(--welcome-surface-3) px-4 py-3 text-sm text-(--welcome-body-text)">{review_form.message}</p>}
                        </div>
                    </div>
                        ...
                        <div className="rounded-[40px] border border-(--welcome-border-soft) bg-(--welcome-surface-1) p-8">
                            <div className="flex flex-wrap items-start justify-between gap-4">
                                <div>
                                    ...
                                    <h2 className="mt-3 font-['Playfair_Display',serif] text-3xl">What buyers are saying</h2>
                                </div>
                        ...
                        <div className="rounded-[40px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-8">
                            <div className="flex items-center justify-between gap-4">
                                <div>
                                    ...
                                    <h2 className="mt-3 font-['Playfair_Display',serif] text-3xl">Review notes from fulfilled orders</h2>
                                </div>
                            </div>
                            ...
                            {reviews.length === 0 ? (
                                ...
                                <div className="mt-6 grid gap-4">
                                    {reviews.map((review) => (
                                        <article key={review.id} className="rounded-[32px] bg-(--welcome-surface-1) p-6">
                                            <div className="flex flex-wrap items-center justify-between gap-4">
                                                <div>
                                                ...
                                                <div className="flex items-center gap-1 text-base">{renderRatingStars(review.rating)}</div>
                                            </div>
                ...
                <Dialog open={isInquiryDialogOpen} onOpenChange={setIsInquiryOpen}>
                    <DialogContent className="max-h-[92vh] overflow-y-auto border-(--welcome-border-soft) bg-(--welcome-surface-1) p-0 sm:max-w-2xl">
                        <DialogHeader className="sr-only">
                            ...
                            <DialogDescription>Send an inquiry directly to this vendor about the selected product.</DialogDescription>
                        </DialogHeader>
                        <div className="p-8">
                            <VendorInquiryForm
                    ...
                    <div className="rounded-4xl bg-(--welcome-surface-1) shadow-[0_24px_70px_-45px_var(--welcome-shadow-strong)]">
                        <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1.06fr)_minmax(360px,0.94fr)]">
                            <div className="overflow-hidden">
                                ...
                                >
                                    {currentImage ? (
                                        ...
                                        <div className="flex h-112 items-center justify-center bg-(--welcome-surface-3) text-sm tracking-[0.3em] text-(--welcome-muted-text) uppercase md:h-[36rem]">
                                            Image forthcoming
                                        </div>
                                    )}
                            ...
                            <div className="space-y-6 p-6 md:p-8">
                                ...
                                <div className="grid gap-3 rounded-[22px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-4 sm:grid-cols-3">
                                    <div>
                                ...
                                <form onSubmit={submit} className="grid gap-3 rounded-[22px] border border-(--welcome-border-soft) bg-(--welcome-surface-2) p-4">
                                    {product.variations.length > 0 && (
                                        <div className="grid gap-2">
                                            <p className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase">
                                ...
                                <div className="grid gap-3 sm:grid-cols-2">
                                    <div className="rounded-[22px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-4 sm:col-span-2">
                                        <p className="text-[11px] tracking-[0.24em] text-(--welcome-muted-text) uppercase">Ingredients</p>
                                        <p className="mt-2 max-w-2xl text-sm leading-6 text-(--welcome-strong)">{product.materials ?? 'Listed on request'}</p>
                                    </div>
                                    <div className="rounded-[22px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-4">
                                    ...
                                    {product.expiry_information && (
                                        <div className="rounded-[22px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-4 sm:col-span-2">
                                            <p className="text-[11px] tracking-[0.24em] text-(--welcome-muted-text) uppercase">Best before / expiry</p>
                                            <p className="mt-2 text-sm text-(--welcome-strong)">{product.expiry_information}</p>
                                        </div>
                                    )}
                                </div>
