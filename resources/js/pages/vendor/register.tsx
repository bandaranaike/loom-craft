            ...
            >
                <section className="relative z-10 mx-auto grid w-full max-w-6xl gap-10 px-6 pt-4 pb-16 lg:grid-cols-[1.05fr_0.95fr]">
                    <div className="space-y-6">
                        <p className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase">Vendor Application</p>
                        <h1 className="font-['Playfair_Display',serif] text-4xl leading-tight md:text-5xl">Apply to sell heritage pieces on LoomCraft.</h1>
                        <p className="max-w-xl text-sm text-(--welcome-body-text) md:text-base">
                            Vendor approval is manual to preserve craft quality and provenance. Start with the compulsory studio name now, then complete the full public profile
                            after creation.
                        </p>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="rounded-[28px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-5">
                                <p className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase">Marketplace Standards</p>
                                <p className="mt-3 text-sm text-(--welcome-body-text)">
                                    Listings follow LoomCraft quality checks for heritage, presentation, and artisan authenticity.
                                </p>
                            </div>
                            <div className="rounded-[28px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-5">
                                <p className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase">Approval Process</p>
                                <p className="mt-3 text-sm text-(--welcome-body-text)">Every vendor application is reviewed by LoomCraft curators.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        <div className="relative rounded-[36px] border border-(--welcome-border) bg-(--welcome-surface-1) p-8 shadow-[0_30px_80px_-45px_var(--welcome-shadow)]">
                            <div className="space-y-2">
                                <p className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase">Vendor Profile</p>
                                <h2 className="font-['Playfair_Display',serif] text-2xl">Registration details</h2>
                                <p className="text-sm text-(--welcome-body-text)">Submit the minimum details required to open your vendor record.</p>
                            </div>
                            ...
                            <Form {...store.form()} className="mt-6 grid gap-5">
                                {({ processing, errors }) => (
                                    <>
                                        <div className="grid gap-2">
                                            <label htmlFor="display_name" className="text-xs font-semibold tracking-[0.3em] text-(--welcome-muted-text) uppercase">
                                                Display name
                                            </label>
                                            <input id="display_name" type="text" name="display_name" placeholder="Studio name" className={inputClassName} required />
                                        ...
                                        <p className="text-center text-xs tracking-[0.25em] text-(--welcome-muted-text) uppercase">
