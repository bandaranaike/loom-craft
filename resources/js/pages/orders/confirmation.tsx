                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <h1 className="font-['Playfair_Display',serif] text-4xl leading-tight md:text-5xl">
                                {order.public_id ?? `Order #${order.id}`} secured
                            </h1>
                                ...
                                {order.items.map((item) => (
                                    <div key={item.id} className="space-y-1">
                                        ...
                                        <p className="text-sm text-(--welcome-strong)">
                                            Line total {formatMoney(item.line_total, order.currency)}
                                        </p>
                                    </div>
                                ))}
                            </div>
                        ...
                        {order.payment_method === 'bank_transfer' && (
                            <div className="rounded-[28px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-6">
                                ...
                                <p className="mt-3 text-sm text-(--welcome-body-text)">
                                    Upload the final transfer slip for {order.public_id ?? `order #${order.id}`}. This page already shows
                    ...
                    <aside className="rounded-[32px] border border-(--welcome-border) bg-(--welcome-surface-1) p-6 shadow-[0_30px_80px_-45px_var(--welcome-shadow)]">
                        ...
                        <div className="mt-4 space-y-3 text-sm">
                            <div className="flex items-center justify-between">
                                ...
                                <span>{order.public_id ?? `#${order.id}`}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="uppercase tracking-[0.3em] text-(--welcome-muted-text)">
                                    Subtotal
                            ...
                            {order.payment_amount && order.payment_currency && (
                                <div className="flex items-center justify-between">
                                    ...
                                    <span>{formatMoney(order.payment_amount, order.payment_currency)}</span>
                                </div>
                            )}
                        ...
                        <div className="mt-6 space-y-3">
                            <Link
                                ...
                                className="inline-flex w-full items-center justify-center rounded-full border border-(--welcome-strong) px-4 py-3 text-xs font-semibold uppercase tracking-[0.3em] text-(--welcome-strong) transition hover:bg-(--welcome-strong) hover:text-(--welcome-on-strong)"
                            >
