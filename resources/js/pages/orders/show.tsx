                ...
                <section className="relative z-10 mx-auto grid w-full max-w-6xl gap-8 px-4 pt-4 pb-16 sm:px-6 lg:grid-cols-[1.1fr_0.9fr]">
                    <div className="min-w-0 space-y-6">
                        <div className="rounded-[28px] border border-(--welcome-border) bg-(--welcome-surface-1) p-7 shadow-[0_20px_50px_-36px_var(--welcome-shadow-strong)]">
                            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                ...
                                <div className="min-w-0 space-y-3 text-left md:text-right">
                                    <p className="font-['Playfair_Display',serif] text-3xl text-(--welcome-strong)">{formatMoney(order.total, order.currency)}</p>
                                    {auth.user && (
                                        <Link href={ordersIndex()} className="text-xs tracking-[0.3em] text-(--welcome-muted-text) uppercase underline">
                                            Back to orders
                                        </Link>
                                    )}
                                </div>
                        ...
                        <div className="rounded-[28px] border border-(--welcome-border-soft) bg-(--welcome-surface-3) p-6">
                            ...
                            <div className="mt-4 space-y-3">
                                {order.items.map((item) => (
                                    <div key={item.id} className="rounded-[22px] border border-(--welcome-border) bg-(--welcome-surface-1) p-4 text-sm">
                                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                            <div className="min-w-0">
                                                <p className="font-semibold text-(--welcome-strong)">{item.product_name}</p>
                                            ...
                                            <div className="text-base font-semibold text-(--welcome-strong)">{formatMoney(item.line_total, order.currency)}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            {shipping && <OrderAddressCard address={shipping} />}
                            {billing && <OrderAddressCard address={billing} />}
                        </div>
